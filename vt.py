...
# --- PATCH: Winhash big/small/draw integration ---
def normalize_headers_for_winhash(headers):
    new_headers = headers.copy()
    if "User-Id" in new_headers:
        new_headers['user-id'] = new_headers.pop("User-Id")
    if "User-Secret-Key" in new_headers:
        new_headers['user-secret-key'] = new_headers.pop("User-Secret-Key")
    new_headers.setdefault('accept', '*/*')
    new_headers.setdefault('origin', 'https://winhash.io')
    new_headers.setdefault('user-agent', random.choice(USER_AGENTS))
    return new_headers

def get_hourly_issue_list_winhash(headers, session=None):
    s = session if session else requests
    try:
        params = {'ts': int(time.time())}
        r = s.get('https://api.winhash.net/lucky_game/hourly_issue_list',
                  params=params,
                  headers=normalize_headers_for_winhash(headers),
                  timeout=15)
        if r.status_code == 200:
            d = r.json()
            if d.get('code') == 0:
...
def find_current_issue_id_winhash(headers, session=None):
    issues = get_hourly_issue_list_winhash(headers, session=session)
    if not issues:
        return None
    ...
    mindiff = float('inf')
    for it in issues:
        diff = abs(current_time - it.get('start_time', 0))
        if diff < mindiff:
            mindiff = diff
            closest = it
    if closest:
...
def place_bet_winhash(headers, asset, issue_id, bet_type, amount, session_obj=None):
    s = session_obj if session_obj else requests
    headers_norm = normalize_headers_for_winhash(headers)
    ...
    time.sleep(random.uniform(1.0, 4.0))  # delay nhỏ
    try:
        r = s.post('https://api.winhash.net/lucky_game/v2/create_order',
                   headers=headers_norm, json=payload, timeout=12)
        if r.status_code == 200:
            data = r.json()
            if data.get('code') == 0:
                print(Fore.GREEN + f"✅ Đặt cược Winhash: {amount} {asset} - {bet_type} (Kì {issue_id})")
...
if __name__ == "__main__":
    # Thêm lệnh gọi display_ascii_art từ toolvth.py
    display_ascii_art()
    ...
    # ======================= NHẬP HỆ SỐ NHÂN X =======================
    print(Fore.CYAN + "═" * 48)
    print(Fore.YELLOW + "   NHẬP HỆ SỐ NHÂN X")
    print(Fore.CYAN + "═" * 48)

    try:
        bet_amount = float(input(Fore.YELLOW + "👉 Nhập số tiền cược ban đầu mỗi trận: " + Fore.WHITE).strip())
        x_factor = float(input(Fore.YELLOW + "👉 Nhập X (hệ số nhân cho 1 ván, mặc định 1): " + Fore.WHITE).strip() or "1")
        print(Fore.GREEN + f"✅ Hệ số X = {x_factor}, sẽ áp dụng nhân tiền cược 1 lần duy nhất")
    except ValueError:
        bet_amount = 10.0
        x_factor = 1.0
        print(Fore.RED + "❌ Nhập sai. Dùng mặc định: bet=10, X=1")
    ...
    x_used = False  # chỉ dùng X một lần
    SAFE_THRESHOLD = 60.0   # % an toàn tối thiểu
    MAX_DANGEROUS = 3       # nếu quá 3 phòng <50% thì nghỉ
    ...
    while True:
        ...
        if pending_issue and str(pending_issue) == str(current_issue):
            ...
            if killed_room_id != pending_room:
                ...
                wait_loss = True
                print(Fore.MAGENTA + "đang quan sát kì...")
        ...
        # ======= AI QUYẾT ĐỊNH NGHỈ =========
        if sorted_rooms:
            if sorted_rooms[0][1] < SAFE_THRESHOLD:
                print(Fore.MAGENTA + f"⏸️ Tỉ lệ an toàn thấp ({sorted_rooms[0][1]:.1f}%), bỏ qua kỳ này...")
                time.sleep(2)
                continue

            dangerous_rooms = [r for r, rate in sorted_rooms if rate < 50]
            if len(dangerous_rooms) >= MAX_DANGEROUS:
                print(Fore.MAGENTA + "⏸️ Quá nhiều phòng nguy hiểm (<90%), nghỉ kỳ này...")
                time.sleep(2)
                continue
        ...
        if sorted_rooms:
            ...
            available_rooms = [(rid, rate) for rid, rate in sorted_rooms
                               if locked_rooms.get(str(rid), 0) == 0
                               and str(rid) != str(killed_room_id)
                               and str(rid) != str(pending_room)]
            ...
            # ========== ÁP DỤNG X-FACTOR ==========
            if not x_used and x_factor > 1:
                bet_for_this_round = current_bet_amount * x_factor
                x_used = True
                print(Fore.MAGENTA + f"⚡ Áp dụng hệ số X={x_factor}, cược {bet_for_this_round} {asset_mode}")
            else:
                bet_for_this_round = current_bet_amount
            if wait_loss:
                print(Fore.MAGENTA + "quan sát kì  ko cược")
                wait_loss = False
                skip_rounds = 3
                continue

            print(Fore.CYAN + f"💰 Số tiền cược cho kỳ {pred_id}: {bet_for_this_round} {asset_mode}")
            success = place_bet(headers, asset_mode, pred_id, int(best_room_id), bet_for_this_round)
            if success:
