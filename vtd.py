...
def place_bet(headers, bet_group, asset_type, athlete_id, issue_id, bet_amount=1.0):
    ...
    payload = {
        ...
        "bet_group": bet_group,       # winner / not_winner
        "asset_type": asset_type,     # BUILD / USDT / WORLD
    ...
    try:
        ...
        if data.get("code") in (0, 200):
            print(Fore.GREEN + f"✅ Đặt cược thành công: {bet_amount} {asset_type} -> {athlete_id}")
            return True
        else:
...
# --- SỬA: bỏ get_current_open_issue, luôn cộng +1 từ current_issue ---
